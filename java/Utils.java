...
public class Utils {
	...
	public static final int MASK_RAW_MAPPED_INT = 0xFFFFF;

	public static final int MASK_ARROW_FLOG = 0x40000000;
	public static final int MASK_FENG_FLOG = 0x100000;
    public static CardType getCardTypeByInt(int mappedInt){
		boolean isArrow = false;
		if((mappedInt & MASK_ARROW_FLOG) != 0){
			isArrow = true;
		}

		boolean isFeng = false;
		if((mappedInt & MASK_FENG_FLOG) != 0){
			isFeng = true;
		}
		if(isArrow){
			return CardType.ARROW;
		}

		if(isFeng){
			return CardType.FENG;
		}
    ...
    public static void getPaiGroupByInt(int mappedInt){
		CardType cardType = getCardTypeByInt(mappedInt);
		int rawMappedInt = (mappedInt & MASK_RAW_MAPPED_INT);
		PaiGroup paiGroup = new PaiGroup(cardType, rawMappedInt);
		int j = 0;
